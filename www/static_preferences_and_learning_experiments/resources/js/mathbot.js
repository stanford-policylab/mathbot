...
function MathbotFSM(
	...
	self.test_questions = ['1-1', '1-2', '2-1', '2-2', '2-3', '4-1', '4-2', '4-2b', '4-3', '4-4', '4-5'] // only pull bandit after hitting these (don't include last question)
	...
	//contextual bandit. Given a context returns an action. 
	function policy(){
		//mooclet id is 101
        //return([])
		//return(['skip_concept'])
    ...
    function moveAndSay(response) {
    	...
    	if (ind >= 0){
    	    ...
    	    if (self.test_questions.length > 0){ // don't turn the bandit on after getting to the last test question
        ...
        if (nextProp && nextProp['nextType']) {
            ...
            if (self.unfinishedQuestionStack.length === 0 &&
                ...
                var forwardNodeId;
                if (self.rootNodeVisitCount[nextNodeId] >= 4) {
    	...
    	//if the bandit flag is on and  we are hitting a root node we've never seen before
        //we are progressing to a new concept or question.
		//todo take the thing we didn't actually go to out of the stack
    	if (self.bandit_flag  && self.fsmJson[self.currNodeId]['isRoot'] && self.rootNodeVisitCountIncludingReview[self.currNodeId] == 1){
			...
			*/
			...
			var test_q_time = self.test_question_finish_times[self.last_test_question] - self.test_question_start_times[self.last_test_question]
			
			console.log('the time is took to finish the last test question was')
			console.log(test_q_time)
			
    	    var action = policy() //policy should return a list of actions
